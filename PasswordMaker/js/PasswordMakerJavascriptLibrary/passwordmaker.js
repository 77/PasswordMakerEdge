...
function preGeneratePassword() {
    ...
    while (password.length < passwdLength.value) {
        password += (count == 0) ?
            generatepassword(hashAlgorithm, passwdMaster.value, passwdUrl.value + usernameTB.value + counter.value, whereToUseL33t, l33tLevel, passwdLength.value, selectedChar, passwordPrefix.value, passwordSuffix.value)
            :
            generatepassword(hashAlgorithm, passwdMaster.value + '\n' + count, passwdUrl.value + usernameTB.value + counter.value, whereToUseL33t, l33tLevel, passwdLength.value, selectedChar, passwordPrefix.value, passwordSuffix.value);
        count++;
    }
    if (passwordPrefix.value)
        password = passwordPrefix.value + password;
    if (passwordSuffix.value)
            password = password.substring(0, passwdLength.value - passwordSuffix.value.length) + passwordSuffix.value; passwdGenerated.value = password.substring(0, passwdLength.value);
}
...
function generatepassword(hashAlgorithm, key, data, whereToUseL33t, l33tLevel, passwordLength, charset, prefix, suffix) {
    ...
    if (whereToUseL33t == "both" || whereToUseL33t == "before-hashing") {
        ...
        if (usingHMAC) {
            data = PasswordMaker_l33t.convert(l33tLevel, data);
        }
    }
    var password = "";
...
function saveProfile() {
    var profileIndex = document.getElementById("profileLB").selectedIndex; var selectedProfile = document.getElementById("profileLB").options[profileIndex].text; if (selectedProfile == "profileList" || selectedProfile == "globalPrefs")//user can't name a profile profileList!!
    {
...
var keySz = keySizeInBits / 4;
function makeKey() {
    var ret = "";
    while (ret.length < keySz)
        ret += hex[Math.floor(Math.random() * 15)]; return ret;
}
...
function in_array(needle, haystack, returnIndex) {
    var n = haystack.length; for (var i = 0; i < n; i++) {
...
if (window.addEventListener) { window.addEventListener('load', init, false); } else if (window.attachEvent) { window.attachEvent('onload', init); } else { var otherOnLoadHandler = window.onload; onload = init; }